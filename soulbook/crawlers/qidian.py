...
class QidianCrawler(BaseCrawler):
    ...
    async def search_novels(self, keyword: str) -> List[Dict[str, str]]:
        """
        Search novels on Qidian by keyword
        """
        search_url = f"https://www.qidian.com/search?kw={keyword}"
    ...
    async def get_novel_info(self, novel_url: str) -> Dict[str, str]:
        """
        Get detailed novel information from Qidian
        """
        ...
        try:
            ...
            author = soup.select_one('.book-info .writer')
            desc = soup.select_one('#bookSummary content')
            cover = soup.select_one('#bookImg img')
    ...
    async def get_chapters(self, novel_url: str) -> List[Dict[str, str]]:
        ...
        """
        # Modify URL to go to catalog page
        catalog_url = novel_url.replace('.html', '/catalog')
        html = await self.fetch(catalog_url)
        ...
        chapter_items = soup.select('.volume-wrap .chapter-list li a')
    ...
    async def get_chapter_content(self, chapter_url: str) -> str:
        ...
        content_div = soup.select_one('.read-content')
        if content_div:
            return content_div.get_text(strip=True)
