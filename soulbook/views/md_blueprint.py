...
from soulbook.fetcher.cache import get_the_latest_chapter, cache_owllook_search_ranking, cache_others_search_ranking
...
async def admin_setting(request):
    ...
    if user:
        try:
            ...
            if data:
                return template('admin_setting.html', title='{user}的设置 - SoulBook'.format(user=user),
                                is_login=1,
                                user=user,
...
async def book_list(request):
    ...
    if user:
        try:
            return template('admin_book_list.html', title='{user}的书单 - SoulBook'.format(user=user),
                            is_login=1,
                            user=user)
        except Exception as e:
            LOGGER.error(e)
            ...
            if data:
                ...
                if books_url:
                    ...
                    for i in books_url:
                        ...
                        result.append(item_result)
                    return template('admin_books.html', title='{user}的书架 - SoulBook'.format(user=user),
                                    is_login=1,
                                    user=user,
                                    is_bookmark=1,
                                    result=result[::-1])
...
async def index(request):
    ...
    if user:
        return template('index.html', title='SoulBook', is_login=1, user=user, search_ranking=search_ranking,
                        first_type=first_type, first_type_title=first_type_title, novels_head=novels_head, is_owl=1)
    else:
        return template('index.html', title='SoulBook', is_login=0, search_ranking=search_ranking, first_type=first_type,
    ...
    if user:
        try:
            ...
            if data:
                ...
                return template('noti_book.html', title='新书提醒 - SoulBook'.format(user=user),
                                is_login=1,
                                is_author=is_author,
